...
迁移入口会忽略 `/Game/ExternalAsset` 和 `/Game/Developer` 本身及其子目录中的引用：

- 若资产只被这些目录中的对象引用，视为无需迁移。
- 若资产同时被其他目录中的对象引用，移动目标使用过滤后的第一个外部引用者所在目录。
- 该规则只影响迁移入口，不改变三个只读检查入口的报告结果。
...
```text
C:\path\to\ball-maze-tools\ue\ue-folder-reference-checker\check_external_references_both.py
```
...
- 留空：每次运行时弹出系统保存框。默认文件名包含检查模式、被检查文件夹名和时间戳，例如 `referenced_by_external-Developer-20260603-140816.csv`。
...
`Referenced By` 检查使用 Unreal 的 package referencer confirmation API，并启用 `load_assets_to_confirm=True`，以更接近删除资产/文件夹前确认弹窗中的 Asset Referencers 结果，减少缓存依赖带来的假阳性。`Referencing` 检查仍使用 Asset Registry 的包级 dependency 查询，不加载资产对象。`SearchableName` 引用默认不计入报告。
